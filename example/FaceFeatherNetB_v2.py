...
class FaceFeatherNet(nn.Module):
    def __init__(self, n_class=2, input_size=112, se=False, avgdown=False, width_mult=1.):
...
def FaceFeatherNetB_v2(se=True):
    model = FaceFeatherNet(se = se,avgdown=True)
    return model
...
#     model = FaceFeatherNetB_v2(se=False)   # Total Flops(Conv Only): 70.46MFlops, model size = 1.35MB
#     print(model)
#
