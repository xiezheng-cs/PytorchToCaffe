...
class PrunedMobileNetV1(nn.Module):
    """
    MobileNetV1 on ImageNet
    """

    def __init__(self, num_classes=1000, pruning_rate=1.0):
        super(PrunedMobileNetV1, self).__init__()

        # define network structure

        self.layer_width = np.array([32, 64, 128, 256, 512, 1024])
        self.layer_width = self.layer_width - np.floor(self.layer_width * pruning_rate)
